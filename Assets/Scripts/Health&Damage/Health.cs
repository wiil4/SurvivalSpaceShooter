...
public class Health : MonoBehaviour
{
    ...
    {
        if (currentLives + 1 > maximumLives)
            return;
        currentLives++;
        FindObjectOfType<UILivesContainer>().AddLife();
    }
    ...
    void HandleDeathWithLives()
    {
        ...
        {
            Respawn();
            FindObjectOfType<UILivesContainer>().TakeLife();
        }
        else
        {
    ...
    void HandleDeathWithoutLives()
    {
        ...
        {
            GetComponent<SpaceStationDestroy>().InstancePowerUp();
        }
        Destroy(this.gameObject);
    }
}
